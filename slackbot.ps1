...
function Get-Leaderboard($CallApi, $Year, $Session, $LeaderboardId) {
    if ($CallApi) {
        ...
        $response = Invoke-WebRequest -Method Get -Uri $url -UserAgent "PowerShell Slack-Integration (maartenvangolen@gmail.com) - homebrew script" -Headers @{ "Cookie" = "session=$Session" }
